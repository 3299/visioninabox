...
with open('calibration.json') as data_file:
  savedCalibration = json.load(data_file)
...
class Vision(object):
  def __init__(self):
    ...
    self.cameraMatrix = np.array(savedCalibration['matrix'], dtype=np.float)

    self.distortionMatrix = np.array(savedCalibration['distortion'], dtype=np.float)
